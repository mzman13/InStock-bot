...
class Gamestop:
    ...
    def monitor(self, productUrl, worker):
        self.setLogger(configureLogger(f"Gamestop-{getProcessNum(multiprocessing.current_process().name)}"))
        ...
        headers = {
            ...
            "Referer": "https://www.gamestop.com/video-games/switch",
            ...
            with requests.Session() as session:
                try:
                    ...
                    if not "Not Available" in response.text:
                        self.logger.info(f"Gamestop - {product} IN STOCK!!! sending message")
                        with worker.lock:
                            alert()
