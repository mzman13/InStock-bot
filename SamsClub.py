...
class SamsClub:
    ...
    def monitor(self, productUrl, worker):
        self.setLogger(configureLogger(f"SamsClub-{getProcessNum(multiprocessing.current_process().name)}"))
        ...
        # for i in range(1):
            with requests.Session() as session:
                try:
                    ...
                    if 'onlineInventory":{"status":"inStock' in response.text:
                        self.logger.info(f"SamsClub - {product} IN STOCK!!! sending message")
                        with worker.lock:
                            alert()
                    ...
                    # else:
                    #     self.logger.info(f"SamsClub - Out of stock for {product}!")
                except:
