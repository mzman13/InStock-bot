...
class Target:
    ...
    def monitor(self, productUrl, worker):
        self.setLogger(configureLogger(f"Target-{getProcessNum(multiprocessing.current_process().name)}"))
        ...
        # for i in range(1):
            with requests.Session() as session:
                try:
                    ...
                    product = productUrl[25:productUrl[25:].find('/') + 25]
                    if "Add to cart" in response.text:
                        self.logger.info(f"Target - {product} IN STOCK!!! sending message")
                        with worker.lock:
                            alert()
                    ...
                    # else:
                    #     self.logger.info(f"Target - Out of stock for {product}!")
                except:
